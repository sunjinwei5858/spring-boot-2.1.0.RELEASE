...
/**
 * DispatcherServletAutoConfiguration类主要包含了两个内部类
 * 1。DispatcherServletConfiguration：配置DispatcherServlet
 * 2。DispatcherServletRegistrationConfiguration： 配置DispatcherServlet注册类
 * 什么是注册类？我们知道Servlet实例是要被添加（注册）到如tomcat这样的ServletContext里的，这样才能够提供请求服务。

...
/**
 * 外部属性绑定配置的后置处理器1--作用：存储工厂bean的元数据 是为了给后置处理2进行使用的
 * <p>
 ...
 * @since 1.1.0
 */
public class ConfigurationBeanFactoryMetadata implements BeanFactoryPostProcessor { // 实现了BeanFactoryPostProcessor
     ...
     * 存储工厂的元数据
     * key：某个bean的名字
     * value：封装相关工厂元数据的FactoryMetadata对象（封装了工厂bean名和工厂方法名）
     */
    private final Map<String, FactoryMetadata> beansFactoryMetadata = new HashMap<>();

    /**
     * 实现了postProcessBeanFactory方法,这个后置方法主要是进行存储beansFactory的元数据
     * 将工厂Bean（可以理解为@Configuration注解的类）及其@Bean注解的工厂方法的一些元数据缓存到beansFactoryMetadata集合中，以便后续使用
     *
     * @param beanFactory
     ...
     * 静态内部类 工厂元数据
     *
     *  比如下面的由@Configuration 修饰的ConfigA，
     *  @Configuration
     ...
     *  }
     *  key："methodB"
     *  value：FactoryMetadata（configA, methodB）对象
     */
